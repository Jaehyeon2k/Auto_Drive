...
def region_of_interest(image):
    ...
    # 도로 영역을 정의하는 다각형 좌표 설정 (왼쪽과 오른쪽 넓게 포함)
    polygons = np.array([[
        (int(0.0 * width), height),              # 왼쪽 아래
        (int(1.0 * width), height),              # 오른쪽 아래
        (int(0.7 * width), int(height * 0.45)),  # 오른쪽 위
        (int(0.3 * width), int(height * 0.45))   # 왼쪽 위
    ]], np.int32)
...
def draw_lines(image, lines):
    ...
    if lines is not None:
        for line in lines:
            ...
            # 수직이거나 너무 짧은 선은 제외
            if abs(y2 - y1) > 20 and abs(x2 - x1) > 20:
...
while cap.isOpened():
    ...
    edges = cv2.Canny(gray, 40, 120)  # Canny 엣지 검출 (하한값 40, 상한값 120)
    ...
    lines = cv2.HoughLinesP(
        ...
        1,                  # 거리 해상도
        np.pi / 180,        # 각도 해상도 (1도)
        threshold=30,       # 직선으로 판단하기 위한 최소 교차점 수
        minLineLength=40,   # 최소 직선 길이
        maxLineGap=40       # 선 간 최대 허용 간격
    )
